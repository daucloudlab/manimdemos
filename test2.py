...
class ValueTrackerDemo(Scene):
    def construct(self):
        ...
        circle = always_redraw(lambda : Circle(radius = r.get_value(), stroke_color = YELLOW,
        stroke_width=5))
        ...
        .set_length(2*r.get_value()*3.1415).next_to(circle, DOWN, buff=0.2))
        triangle = always_redraw(lambda:Polygon(circle.get_left(), circle.get_top(), circle.get_right(),
        fill_color=GREEN_C))

        self.play(LaggedStart(Create(circle), DrawBorderThenFill(line_radius),
